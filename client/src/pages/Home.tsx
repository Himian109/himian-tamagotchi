 ...
 */
import HomeCharacterParade from "@/components/HomeCharacterParade";
    ...
    <SiteShell mainClassName="home-main">
      <section className="relative overflow-hidden border-b-[3px] border-[#23396b] bg-[#fff8e7]">
        <div className="container grid min-h-[700px] gap-10 py-14 lg:grid-cols-[.92fr_1.08fr] lg:items-center lg:py-20">
          ...
          <div className="relative mt-4 min-h-[390px] isolate sm:min-h-[460px] lg:mt-0 lg:min-h-[560px]">
            ...
            <HomeCharacterParade names={homePals} />
          ...
          <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {categories.map((category, index) => (
              ...
              >
                ...
                {categoryPals[index] && <img src={categoryPals[index]?.artwork} alt="" className="absolute -bottom-4 -right-4 h-28 w-28 rotate-6 object-contain transition-transform group-hover:scale-110" loading="lazy" />}
              </Link>
            ))}
          </div>
        </div>
      ...
      <section className="py-20 sm:py-28">
        <div className="container grid gap-10 lg:grid-cols-[1.1fr_.9fr] lg:items-start">
          <div>
            <p className="mono-type text-xs font-bold text-[#a54842]">GUIDE MAP / START HERE</p>
            <h2 className="display-type mt-3 text-4xl font-black sm:text-5xl">第一次玩，先走這條。</h2>
            <p className="mt-5 max-w-2xl text-base leading-8 text-[#56706a]">從蛋、場地、照顧到進化，一次理解最重要的遊玩循環。</p>
          </div>
          <div className="grid gap-3 sm:grid-cols-2">
            {guideSections.slice(0, 4).map((item, index) => (
              <Link key={item.id} href={`/guide#${item.id}`} className="group focus-ring rounded-2xl border-2 border-[#23396b] bg-white p-5 shadow-[0_4px_0_#23396b] hover:-translate-y-1">
                <span className="mono-type text-[10px] font-black text-[#a54842]">0{index + 1}</span>
                <h3 className="mt-3 text-lg font-black">{item.title}</h3>
                <p className="mt-2 text-sm leading-6 text-[#60756f]">{item.description}</p>
                <span className="mt-4 inline-flex items-center gap-1 text-xs font-black text-[#ff6f61]">閱讀 <ArrowRight className="h-3 w-3 transition-transform group-hover:translate-x-1" /></span>
              </Link>
            ))}
          </div>
        </div>
